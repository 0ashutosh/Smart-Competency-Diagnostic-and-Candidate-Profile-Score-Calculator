...
def run():
    ...
    link = '<b>Built with 🤍 by <a href="https://github.com/0ashutosh/SMART-COMPETENCY-CALCULATOR" style="text-decoration: none; color: #5C7AEA;">Team CSD_5</a></b>' 
...
###### CODE FOR INTERVIEW BOT ######
        ...
        def evaluate_answer(user_ans: str, ref_ans: str) -> tuple[float, str, float, float, list]:
            ...
            if not user_ans:
                return 0.0, "❌ No answer provided.", 0.0, 0.0, []
            ...
            # Feedback text
            if final >= 80:
                fb = "✅ Excellent. You captured the meaning and key terms."
            elif final >= 60:
                fb = "🟡 Decent attempt. Some meaning overlaps, but add more detail/terms."
            else:
                fb = "❌ Not quite there. Expand your explanation and include core concepts."

            if missing:
                fb += f"\n\n🔎 Missing important terms: {', '.join(missing[:5])}"
        ...
        if curr < total_q:
            ...
            if not res_df.empty:
                ...
                st.download_button(
                    "⬇️ Download interview report (CSV)",
                    data=csv,
